...
class SegFormerRGBPEPBackbone(nn.Module):
    """
    SegFormer-B2 backbone for RGB + PEP binary segmentation.
    ...
    def __init__(self, num_labels: int = 1):
        ...
        # Project the 4-channel input (RGB + PEP) to 3 channels for SegFormer.
        self.stem = nn.Sequential(
            nn.Conv2d(4, 32, kernel_size=1, bias=False),
            nn.BatchNorm2d(32),
            nn.GELU(),
    ...
    def forward(self, rgb: torch.Tensor, pep: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for RGB and PEP inputs.
        ...
        x = torch.cat([rgb, pep], dim=1)  # [B, 4, H, W]
...
class SegFormerRGBPEPRunner(ModelRunner):
    """
    Runner wrapper for SegFormer with RGB + PEP input.
    """

    def __init__(self, load_path: Optional[str] = None, use_data_parallel: bool = False):
        super().__init__()
        self.model_ = SegFormerRGBPEPBackbone(num_labels=1)
