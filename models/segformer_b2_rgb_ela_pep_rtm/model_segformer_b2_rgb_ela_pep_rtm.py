...
class SegFormerRGBELAPEPBackbone(nn.Module):
    """
    SegFormer-B2 backbone for RGB + ELA + PEP binary segmentation.
    ...
    def __init__(self, num_labels: int = 1):
        ...
        # Project the 7-channel input (RGB + ELA + PEP) to 3 channels for SegFormer.
        self.stem = nn.Sequential(
            nn.Conv2d(7, 32, kernel_size=1, bias=False),
            nn.BatchNorm2d(32),
            nn.GELU(),
    ...
    def forward(self, rgb: torch.Tensor, ela: torch.Tensor, pep: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for RGB, ELA, and PEP inputs.
        """
        ...
        x = torch.cat([rgb, ela, pep], dim=1)  # [B, 7, H, W]
...
class SegFormerRGBELAPEPRunner(ModelRunner):
    """
    Runner wrapper for SegFormer with RGB + ELA + PEP input.
    """

    def __init__(self, load_path: Optional[str] = None, use_data_parallel: bool = False):
        super().__init__()
        self.model_ = SegFormerRGBELAPEPBackbone(num_labels=1)
