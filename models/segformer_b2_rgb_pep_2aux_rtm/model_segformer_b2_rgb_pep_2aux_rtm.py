...
class SegFormerRGBPEPAuxBackbone(nn.Module):
    """
    SegFormer-B2 backbone for RGB + PEP binary segmentation with two auxiliary channels.
    ...
    def __init__(self, num_labels: int = 1):
        ...
        # Project the 6-channel input (RGB + PEP + auxiliary channels) to 3 channels for SegFormer.
    ...
    def forward(self, rgb: torch.Tensor, pep: torch.Tensor, aux: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for RGB, PEP, and auxiliary inputs.
        ...
        x = torch.cat([rgb, pep, aux], dim=1)  # [B, 6, H, W]
...
class SegFormerRGBPEPAuxRunner(ModelRunner):
    """
    Runner wrapper for SegFormer with RGB + PEP input and internally derived auxiliary channels.
    """

    def __init__(self, load_path: Optional[str] = None, use_data_parallel: bool = False):
        super().__init__()
        self.model_ = SegFormerRGBPEPAuxBackbone(num_labels=1)
