...
class SegFormerRGBELAAuxBackbone(nn.Module):
    """
    SegFormer-B2 backbone for RGB + ELA binary segmentation with two auxiliary channels.
    ...
    def __init__(self, num_labels: int = 1):
        ...
        # Project the 8-channel input (RGB + ELA + auxiliary channels) to 3 channels for SegFormer.
        self.stem = nn.Sequential(
            nn.Conv2d(8, 32, kernel_size=1, bias=False),
            nn.BatchNorm2d(32),
            nn.GELU(),
    ...
    def forward(self, rgb: torch.Tensor, ela: torch.Tensor, aux: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for RGB, ELA, and auxiliary inputs.
        ...
        x = torch.cat([rgb, ela, aux], dim=1)  # [B, 8, H, W]
...
class SegFormerRGBELAAuxRunner(ModelRunner):
    """
    Runner wrapper for SegFormer with RGB + ELA input and internally derived auxiliary channels.
    """

    def __init__(self, load_path: Optional[str] = None, use_data_parallel: bool = False):
        super().__init__()
        self.model_ = SegFormerRGBELAAuxBackbone(num_labels=1)
