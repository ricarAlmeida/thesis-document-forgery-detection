...
class SegFormerRGBAuxBackbone(nn.Module):
    """
    SegFormer-B2 backbone for RGB-based binary segmentation with two auxiliary channels.
    ...
    def __init__(self, num_labels: int = 1):
        ...
        # Project the 5-channel input (RGB + auxiliary channels) to 3 channels for SegFormer.
    ...
    def forward(self, rgb: torch.Tensor, aux: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for RGB input and auxiliary channels.
        ...
        x = torch.cat([rgb, aux], dim=1)  # [B, 5, H, W]
...
class SegFormerRGBAuxRunner(ModelRunner):
    """
    Runner wrapper for SegFormer with RGB input and internally derived auxiliary channels.
    """

    def __init__(self, load_path: Optional[str] = None, use_data_parallel: bool = False):
        super().__init__()
        self.model_ = SegFormerRGBAuxBackbone(num_labels=1)
