...
class SegFormerRGBELABackbone(nn.Module):
    """
    SegFormer-B2 backbone for RGB + ELA binary segmentation.
    ...
    def __init__(self, num_labels: int = 1):
        ...
        # Project the 6-channel input (RGB + ELA) to 3 channels for SegFormer.
    ...
    def forward(self, rgb: torch.Tensor, ela: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for RGB and ELA inputs.
        ...
        x = torch.cat([rgb, ela], dim=1)  # [B, 6, H, W]
...
class SegFormerRGBELARunner(ModelRunner):
    """
    Runner wrapper for SegFormer with RGB + ELA input.
    """

    def __init__(self, load_path: Optional[str] = None, use_data_parallel: bool = False):
        super().__init__()
        self.model_ = SegFormerRGBELABackbone(num_labels=1)
