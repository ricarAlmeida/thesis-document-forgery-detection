"""Dataset for document forgery localization using ELA + PEP features."""
...
class BlockValues:
    ...
    Attributes:
        ...
        ela: ELA tensor in [0,1], shape [3,H,W]
    ...
    def __post_init__(self):
        ...
        if self.ela is not None:
            assert self.ela.shape[-2:] == self.crop_size
            assert self.ela.shape[0] == 3
...
class DocForgeryELAPEPDataset(Dataset):
    """
    Dataset for document forgery localization using ELA + PEP as auxiliary inputs.
    ...
    5. compute ELA and PEP from the final crop
    """
    ...
    def __init__(
        ...
        assert self._crop_size is not None, "crop_size must be provided, e.g. (512, 512)"
        win = int(self._crop_size[0])
        self.stride = int(stride) if stride is not None else win  # ✅ NOVO
    ...
    def ela_features(
        ...
        """
        Compute ELA features from a PIL image.

        The result is:
        - absolute JPEG reconstruction error
        - normalized to [0,1]
        - contrast-amplified for a stronger forensic signal
        """

        assert 0 < qf_ela <= 100

        tmp_dir = Path("/media/general_storage6/tmp")
        ...
        with tempfile.NamedTemporaryFile(suffix=".jpg", dir=tmp_dir, delete=True) as tmp:
            ...
            image_ela = Image.open(tmp.name).convert("RGB").copy()
        
        ela = np.array(image) - np.array(image_ela)
        ...
        if crop_size is not None:
            ...
            if h < crop_size[0] or w < crop_size[1]:
                ...
                temp[:ela.shape[0], :ela.shape[1], :] = ela
                ela = temp
    
            ela = ela[s_r:s_r + crop_size[0], s_c:s_c + crop_size[1], :]

        t_ela = torch.tensor(ela.transpose(2, 0, 1), dtype=torch.float32).abs()

        # Stable normalization: raw ELA difference is in [0,255].
        t_ela = t_ela / 255.0

        # Contrast amplification to make forensic traces more visible
        t_ela = torch.clamp(t_ela * 10.0, 0.0, 1.0)

        t_ela = torch.nan_to_num(t_ela, nan=0.0, posinf=0.0, neginf=0.0)
        return t_ela

    
    def ela_from_crop_np(self, img_np: np.ndarray, qf_ela: int) -> torch.Tensor:
        """
        Compute ELA features from a final crop image.
        Returns a tensor with shape [3,H,W] in [0,1].
        """
        
        pil = Image.fromarray(img_np).convert("RGB")
        return self.ela_features(
            image=pil,
            origin=(0, 0),
            crop_size=None,
            qf_ela=qf_ela,
    ...
    def __getitem__(self, idx: int):
        ...
        qf_ela = self.QF - 1
        ela = self.ela_from_crop_np(img_crop_np, qf_ela=qf_ela)
    ...
    def pep_features_from_crop_np(
        ...
        try:
            ...
            jpg_path = DocForgeryELAPEPDataset._save_crop_as_jpeg_tmp(
                img_crop_np,
            ...
            pep = DocForgeryELAPEPDataset.pep_features(
