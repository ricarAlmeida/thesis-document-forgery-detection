...
def pad_collate(batch, ignore_index=-1):
    """
    Collate function for variable-size RGB and ELA crops.
    ...
    where:
        ...
        block.ela   -> tensor [3, H, W] or None
    ...
    Returns:
        dict with:
            ...
            ela   -> [B, 3, H_max, W_max] if available
    ...
    # Add ELA only if it is available in the batch.
    if getattr(blocks[0], "ela", None) is not None:
        out["ela"] = torch.stack([_pad(b.ela, 0.0) for b in blocks], dim=0)
