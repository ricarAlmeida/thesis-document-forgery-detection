...
def pad_collate(batch, ignore_index=-1):
    """
    Collate function for variable-size RGB and PEP crops.
    ...
    where:
        ...
        block.pep   -> tensor [1, H, W] or None
    ...
    Returns:
        dict with:
            ...
            pep   -> [B, 1, H_max, W_max] if available
    ...
    # Add PEP only if it is available in the batch.
    if getattr(blocks[0], "pep", None) is not None:
        out["pep"] = torch.stack([_pad(b.pep, 0.0) for b in blocks], dim=0)
